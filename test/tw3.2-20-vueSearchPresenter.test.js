...
describe("TW3.2 Vue stateful Search presenter", function () {
    ...
    it("Search presenter initial test, should also pass with TW2 functional code", async function(){
        ...
        expect(JSON.stringify(resultsProps.slice(-1)[0].searchResults), "search results view should be rendered after promise resolve").to.equal(JSON.stringify([searchResults[1], searchResults[0]]));
    ...
    it("Search presenter initiates a search promise at first render and resolves the promise in component state", async function(){
        ...
        await new Promise(resolve => setTimeout(resolve)); // give the UI a chance to update

        expect(resultsProps.length, "search results view should be rendered after promise resolve").to.equal(1);
        expect(JSON.stringify(resultsProps[0].searchResults), "search results view should be rendered after promise resolve").to.equal(JSON.stringify(searchResults));
