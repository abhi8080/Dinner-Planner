...
class TestAnalyzer {
    ...
    constructor(mochaTestSuites) {
        ...
        mochaTestSuites.forEach(suite => {
            ...
            let isCompletedSuite = suite.tests.some(test => test.state && test.state !== "pending");
            ...
            if (isCompletedSuite)
                this.completedSuites.push(cleanSuite)
        })
    }
...
class MyRunner extends runnerClass {
    ...
    runAsync(...params){ return super.runAsync(...params).then(result=> {
