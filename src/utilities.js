...
function compareIngredientsCB(ingredientA, ingredientB) {
  ...
  else if (ingredientA.aisle < ingredientB.aisle) return -1;
  else {
...
function isKnownTypeCB(type) {
  if (dishTypeRanking[type] !== undefined) return true;
...
function dishType(dish) {
  if (dish["dishTypes"] == undefined) return "";
  let dishType = dish.dishTypes.find(isKnownTypeCB);
  if (dishType == undefined) return "";
  else return dishType;
}
...
function compareDishesCB(dishA, dishB) {
  if (dishTypeRanking[dishType(dishA)] - dishTypeRanking[dishType(dishB)] > 0)
    return 1;
  else if (
    dishTypeRanking[dishType(dishA)] - dishTypeRanking[dishType(dishB)] <
    0
  )
    return -1;
  return 0;
...
function menuPrice(dishesArray) {
  ...
  const arrayOfPrices = dishesArray.map(getPriceCB);
  ...
  return arrayOfPrices.reduce(getSumCB, 0);
}
